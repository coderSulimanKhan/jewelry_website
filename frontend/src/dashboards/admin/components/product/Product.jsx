...
const Product = ({ product }) => {
  ...
  const calculateTotalPrice = (p, f, pp, m) => {
    let totalPrice = 0;
    if (p) {
      totalPrice = p;
    }
  ...
  return (
    <>
      ...
      <td align="center"><span className="bg-warning/20 text-warning rounded text-lg flex items-center justify-center gap-1 mx-2 px-1">{calculateTotalPrice(product?.price, product?.discountFee, product?.discountPercentage, product?.making)} <span className="text-xs text-red-600">PKR</span> </span></td>
