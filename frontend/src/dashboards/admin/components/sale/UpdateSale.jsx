...
const UpdateSale = () => {
    ...
    const { sale: storedSale } = useSelector(state => state.sale);
    ...
    useEffect(() => {
        if (storedSale) {
            setSaleToUpdate(storedSale);
        }
    }, [storedSale]);

    useEffect(() => {
        setUserId(saleToUpdate?.user);
        setAddedItems(saleToUpdate?.items ?? []);
        setIsDefaultProductsDiscounts(saleToUpdate?.isDefaultProductsDiscounts);
        setCash(saleToUpdate?.cash || 0);
        setAnother(saleToUpdate?.another || 0);
        setMoreDiscountFee(saleToUpdate?.discountFee || 0);
        setMoreDiscountPer(saleToUpdate?.discountPercentage || 0);
        setTotalPrice(saleToUpdate?.totalPrice || 0);
    }, [saleToUpdate]);
    ...
    return (
        <div className="flex flex-col gap-10 p-1 w-full">
            ...
            <div className="flex items-center justify-between pr-10">
                <h1 className="adminCardH1">Update Sale</h1>
            ...
            <div className="flex justify-center">
                <form onSubmit={handleFormSubmit} className="w-4/5 border border-error p-3 rounded flex flex-col gap-4 max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="flex gap-3">
                        ...
                        <p className="text-4xl">{totalPrice || 0} <span className="text-warning text-sm">PKR</span> <span className="text-xl bg-warning/20 px-1 rounded-full text-warning">{saleToUpdate?.totalPrice}<span className="text-error text-xs">PKR</span></span></p>
                    </div>
                    <div className="flex gap-3">
                        <label htmlFor="totalPrice" className="adminCardH1">Remaining Fee</label>
                        <p className="text-4xl">{totalPrice - cash - another} <span className="text-warning text-sm">PKR</span> <span className="text-xl bg-warning/20 px-1 rounded-full text-warning">{saleToUpdate?.remainingFee}<span className="text-error text-xs">PKR</span></span></p>
                    </div>
